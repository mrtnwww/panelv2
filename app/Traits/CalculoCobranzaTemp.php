...
{
    public function calculoCobranzaIntMoraTemp($vCreditos, $fecha = null, $masivo = false)
    {
        ...
        foreach ($creditos as $credito) {
            ...
            if ($credito->cliente !== null && $credito->cliente->fecha_fin_acuerdo_pago !== null){
                ...
                if ($fechaCalculo->lessThanOrEqualTo($fechaFinAcuerdoPago)) continue;
            }
            ...
            if(count($proyecciones) > 0 &&  $credito->cliente !== null){
                ...
                if($pagadoEn0){
                    ...
                    foreach ($proyecciones as $proyeccion) {
                        ...
                        if ($fechaCalculo->gt($proyeccionFecha)) {
                            $diferencia = (int) $fechaCalculo->diffInDays($proyeccionFecha, true);

                            if ($diferencia > 0) {
                                // $proyeccion->diasMora = $diferencia; // Dias de mora de la cuota proyectada
                                $datosCalculo = $this->calcularDiasPorMesTemp($diferencia);
                                $totalInteresM = 0;
                                ...
                                $proyeccion->$proyIntereses = max(0, $totalInteresM);

                                // Gastos de cobranza 'PROPIA'
                                $porcentaje = $this->obtenerPorcentajeTemp($diferencia, $credito->empresa_id , 'PROPIA');
                                $valorCuota = is_null($proyeccion->valor_cuota) ? $credito->val_cuotas : $proyeccion->valor_cuota;
                                $valorMora = $valorCuota - $abonoFavor;
                                $abonoFavor = 0;

                                $gastosCobranza = ($porcentaje / 100) * ($valorMora);
                                $proyeccion->$proyGastosCob = max(0, $gastosCobranza);

                                // Gastos de cobranza 'CASA COBRANZA
                                if (!$masivo) {
                                    $porcentaje = $this->obtenerPorcentajeTemp($diferencia, $credito->empresa_id, 'CASA COBRANZA');
                                    $casa_gas_cobranza = ($porcentaje / 100) * ($valorMora);
                                    $proyeccion->casa_gas_cobranza_temp = max(0, $casa_gas_cobranza);
                                }

                                //Actualizar el valor de la mora
                                $proyeccion->$proyValorMora = $valorMora + $proyeccion->$proyGastosCob + $proyeccion->$proyIntereses;
    ...
    private function calcularDiasPorMesTemp($dias) {
        $resultado = [];
    ...
    private function obtenerPorcentajeTemp($numero,$empresa_id , $tipo) {
        if($numero > 0){
